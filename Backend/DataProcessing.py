...
def calculate_accel(data: list, interval: int) -> float:
    if len(data) < (interval * 2) + 1 or data[-1 - interval] == 0 or data[-1 - (2 * interval)] == 0:
        return float("-inf")
...
def tests():
    ...
    cases = (
        ("Testing update ticker today...", update_ticker_data_today, # f(score, data)
         [
            ((5, [1,2,3,4]), [1,2,3,4,5]),
            ((5, []), [5]),
            (
        ...
        ("Testing update ticker history...", update_ticker_data_history, # f(score, expiry, data)
         [
            ((5, 2, [1]), [1, 5]),
            ((5, 2, [1, 2]), [2, 5]),
            ((5, 2, [1, 2, 3]), [3, 5])
        ...
        ("Testing interval calculation...", calculate_interval_change, # f(data, interval)
         [
            (([1, 2, 3, 4, 5], 1), 0.25),
            (([1, 2, 3, 4, 5], 2), 2/3),
            (([1, 2, 3, 5, 4], 1), -0.2),
