...
class ProcessSupervisor:
    ...
    def start_monitoring(self, 
        ...
        try:
            ...
            # Set target tickers from parameter or load from file
            if target_tickers:
                self.target_tickers = set(ticker.lower() for ticker in target_tickers)
                self.logger.info(f"Loaded {len(self.target_tickers)} target tickers from parameter list")
            else:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tickers_completed": health.tickers_completed,
            "total_tickers": health.total_tickers,
            "completion_rate": health.completion_rate,
            "remaining_tickers": len(self.get_remaining_tickers()),
            "all_completed": self._all_tickers_completed()
        }
...
def run_supervised_scraping(username: str, 
    ...
    try:
        if supervisor.start_monitoring(
            ...
            target_tickers=tickers,  # Pass the tickers to monitor
            monitor_file=output_file
        ):
            logger.info(f"Supervised scraping started for {len(tickers)} tickers.")
            ...
            while supervisor.monitor_running:
                ...
                if status.get("all_completed", False):
                ...
                if status.get("tickers_completed", 0) > 0:
                    remaining = status.get("remaining_tickers", 0)
                    completion_rate = status.get("completion_rate", 0)
                    logger.info(f"Progress: {status['tickers_completed']}/{status.get('total_tickers', 0)} completed, {remaining} remaining ({completion_rate:.1%})")
